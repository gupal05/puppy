...
//
//	/*
//	 * RequestContetHolder Class --> Spring 3.0 이상 : ThreadLocal를 사용해서 현재 쓰레드에
//	 * RequestAttributes 인스턴스를 바인딩 해두었다가 요청을 하면 이 인스턴스를 돌려주는 역할을 합니다. : ThreadLocal을
//	 * 사용하는 경우 ThreadLocal 변수에 보관된 데이터의 사용이 끝나면 반드시 삭제 해야 재사용되는 쓰레드의 잘못된 참조를 방지 그렇지
//	 * 않으면, 재사용되는 쓰레드가 올바르지 않은 데이터를 참조할 수 있다.
//	 * 
...
//	}
//
//	public JWTBean convertToBean(String token) {
//		JWTBean tokenInfo = null;
//		Object obj = null;
//		try {
//			if ((obj = this.getAttribute("AccessInfo")) != null) {
//				Map<String, Object> tokenInfoMap = this.jwts.getTokenInfo(token, ((UserBean) obj).getUserCode());
//				ObjectMapper objMap = new ObjectMapper();
//				tokenInfo = objMap.convertValue(tokenInfoMap.get("TokenBody"), JWTBean.class);
//			}
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
...
//	}
//	/*
//	 * JWT 정보와 Session정보 일치 여부 리턴 추가 public boolean isMatchSession(String token) {
//	 * boolean isMatch = false; Object obj = null; try { if((obj =
//	 * this.getAttribute("AccessInfo")) != null) { UserBean accessInfo =
//	 * ((UserBean)obj); JWTBean tokenInfo = this.convertToBean(token);
//	 * if(accessInfo.getUserCode().equals(tokenInfo.getUserId()) &&
//	 * accessInfo.getUserEmail().equals(tokenInfo.getUserEmail())) { isMatch = true;
//	 * } } } catch (Exception e) {e.printStackTrace();} return isMatch; }
//	 */
//
//	/* Response Header 추가 */
//	public void transferJWTByResponse(String jwt) {
//		((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getResponse().setHeader("petmateJWT",
//				jwt);
//	}
//
